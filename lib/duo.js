...
Duo.prototype.dependencies = function *(file, map) {
  ...
  if (isCached) {
    ...
    if (isAsset) this.assets.push(this.bundle(file.id));
  ...
  // maybe the file changed after the
  // transform, so update `isAsset`
 ...
 * Examples:
 *
 *    this.dependency('./one.js', file, entry)
 *      => lib/one/one.js
 *
 *    this.dependency('component/tip', file, entry)
 *      => components/component-tip@master/index.js
 *
 *    this.dependency('yields/skeleton', file, entry)
 ...
 *       => lib/one/one.js
 *
 *   - Remote dependency (after it's installed):
 *
 ...
 * Examples:
 *
 *   - "component/emitter": "0.1.0" in our manifest
 *
 *     var pkg = this.package('emitter', file);
 *     pkg.slug(); => component/emitter@0.1.0
 *
 *   - require('component/emitter@master:index.js')
 *
 ...
 * Examples:
 *
 *   - specific dependency. no `manifest`:
 *
 ...
 *        }
 *
 *   - vague dependency. "component/emitter": "0.1.0" is in the `manifest`:
 *
 ...
 *        }
 *
 *   - specific dependency. "component/emitter": "0.1.0" is in the `manifest`:
 *
