...
Duo.prototype.run = unyield(function *() {
  ...
  var manifestPath = join(this.root, this.manifestName);
  ...
  // Load the manifest
  this.manifest = yield this.json(manifestPath);
...
Duo.prototype.dependency = function *(dep, file, entry) {
  ...
  // remote dependency
  var json = yield this.json(join(file.root, this.manifestName));
...
Duo.prototype.resolve = function *(dep, file, entry) {
  ...
  if (this.manifestName == basename(dep)) {
    // component
    var json = yield this.json(resolve(file.root, dep));
...
Duo.prototype.json = function *(path) {
  if (this.jsons[path]) return this.jsons[path];

  try {
    var str = yield fs.readFile(path, 'utf8');
    var obj = JSON.parse(str);
    return this.jsons[path] = obj;
  } catch(e) {
