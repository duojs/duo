...
Duo.prototype.src = function(src, type) {
  type = type || detect(src);
  type = langmap[type] || type;
  if (!type) throw error('duo.src(src) cannot determine type');
...
Duo.prototype.assets = function(path) {
  this.assetPath = join(this.root, path);
  return this;
};
...
Duo.prototype.write = unyield(function *() {
  throw error('write not supported...');
