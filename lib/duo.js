...
Duo.prototype.resolveDependency = function *(dep, parent) {
  var entry = this.entry();
  var isHTTP = http(dep);
  ...
  // local dependency
  var local = yield this.resolveDependencyPath(dep, parent);
  if (local) return relative(entry.root, local);

  // remote dependency
  var dependencyPackage = this.package(dep, parent);
  ...
  // path
  var manifestPath = parse(dep).path || this.manifest();
...
/**
 * Create a package from a `dep` inside `file`.
 *
 * @param {String} dep
 * @param {File} file Where this `dep` was defined.
 * @return {Package|null}
 * @api private
 */
...
Duo.prototype.package = function (dep, file) {
  ...
  var gh = this.finddeps(parse(dep), manifest);
  if (!gh) return null;
...
Duo.prototype.finddeps = function(gh, manifest) {
  ...
  // go through each of the dependencies in component.json,
  ...
  // if the dependency that was originally passed in
