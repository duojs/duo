...
exports = module.exports = Duo;

/**
 * Export `Mapping`.
 *
 * This will allow the mapping/cache to be shared between multiple builds.
 */
...
Duo.prototype.cleanCache = function *() {
  ...
  yield rm(this.installPath('duo.json'));
};
...
/**
 * Returns the mapping/cache for this build, creates one
...
Duo.prototype.mapping = function (mapping) {
  // used to set mapping externally
  if (mapping) {
    this._mapping = mapping;
    return this;
  }

  // if not already defined, we need to create one
  if (!this._mapping) this._mapping = new Mapping(this.installPath('duo.json'));
...
Duo.prototype.install = unyield(function *() {
  ...
  // add core plugins
  this
    .use(compat({ token: token }))
    .use(stoj());

  // try to load duo.json mapping
  if (this.cache()) this.map = yield mapping.read();
...
Duo.prototype.run = unyield(function *() {
  ...
  yield this.altPlugins.run(results, this.entry());
...
Duo.prototype.dependencies = function *(file, map) {
  ...
  var json = clone(this.map[file.id] || {});
