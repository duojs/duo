...
Duo.prototype.write = yieldable(function *() {
...
Duo.prototype.run = yieldable(function *() {
  ...
  if (!(yield exists(entry))) {
    throw error('cannot find entry: %s', entry);
  }
...
Duo.prototype.dependencies = function *(path, root, out) {
  ...
  var file = new File({ path: path, root: root }, this.file);
...
Duo.prototype.dependency = function *(dep, file) {
  ...
  if (http(dep)) {
    debug('%s: ignoring "%s"', file.id, dep)
    return false;
  }
...
function yieldable(gen) {
  return function(done) {
    return done
      ? co(gen).call(this, done)
      : gen.call(this);
  };
}
