 ...
 * @return {Duo|Generator} fn
 * @api private
 */

Duo.prototype.write = function(fn) {
  return fn
    ? co(this._write).call(this, fn)
    : this._write();
}
...
Duo.prototype._write = function *() {
  var src = yield this.run();
  var rel = relative(this.root, this._entry);
...
Duo.prototype._run = function *() {
  ...
  if (!entry) throw error('cannot find entry: %s', this._entry);
...
Duo.prototype.dependencies = function *(path, root, out) {
  ...
  var file = new File(path, root, this);
  var cache = clone(this.mapping[file.relative] || {});
  var mtime = yield file.mtime();
  var paths = [];
  var gens = [];

  // already visited
  if (out[file.relative]) {
    debug('%s, already parsed. ignoring', file.relative);
    return out;
  }

  // included manually
  if (cache.include) {
    out[file.relative] = cache;
    return out;
  }

  // check if the file has been modified
  if (mtime == cache.mtime) {
    debug('%s: has not been modified. skip parsing', file.relative);
    out[file.relative] = cache;
    ...
    for (var i = 0, path; path = paths[i++];) {
      ...
      gens.push(this.dependencies(path, this.findroot(path), out));
    }
  ...
  //   debug('%s: has not been modified. skip parsing', relativeFile);
  ...
  //   if (entry) throw error('%s: ".%s" not supported', relativeFile, layer.type);
  ...
  out[file.relative] = extend(file.json(), { deps: depmap });
...
Duo.prototype.dependency = function *(dep, file) {
  ...
  if (http(dep)) {
    debug('%s: ignoring "%s"', file.relative, dep)
    return false;
  }
  ...
  if (!pkg) {
    debug('%s: cannot resolve "%s"', file.relative, dep);
    return false;
  }
  ...
  // debug('%s: file deps %j', relativeFile, deps)
  ...
  //     debug('%s: has a mapping for dep "%s"', relativeFile, dep);
  ...
  //   // Ignore http deps
  //   if (http(dep)) {
  //     debug('%s: ignoring "%s"', relativeFile, dep)
  //     continue;
  //   }
  ...
  //     if (!pkg) {
  //       debug('%s: cannot resolve "%s"', relativeFile, dep);
  //       // do not cache the file when there is an
  //       // unresolved dep. usually it's a mistake.
  //       delete layer.mtime;
  //       continue;
  //     }
  //     pkgmap[dep] = pkg;
...
//   debug('%s: transforming (%s)', layer.id, layer.type);
...
//   debug('%s: transformed (%s)', layer.id, layer.type);
...
Duo.prototype.resolve = function*(path, root, file) {
  ...
  if (this.manifestName == basename(path)) {
    ...
    var entry = main(json, this.type);
    return resolve(root, dirname(path), entry);
  } else if ('/' == path[0]) {
