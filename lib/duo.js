...
Duo.prototype._run = function *() {
  ...
  var root = this.root;
  var pack = Pack({ debug: this.dev });

  // add the root extension
  this.type = extname(entry).slice(1);

  // 1) Try to load {installPath}/duo.json mapping
  this.mapping = yield this.readjson(this.mappingPath);
  ...
  yield fs.writeFile(this.mappingPath, stringify(extend(this.mapping, deps)));
...
Duo.prototype.dependencies = function *(file, root, out) {
  ...
  if (isEntry) {
    ...
    if (!rsupported.test(layer.type)) {
      throw new Error(layer.type + ' file type not supported.');
    }
  }
...
Duo.prototype.resolve = function*(path, root, file) {
  var ext = extname(path) ? '' : '.' + this.type;
  var ret;
