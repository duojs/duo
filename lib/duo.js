...
Duo.prototype.run = function *() {
  ...
  entry = this.resolve(entry || this.manifestPath, root);
...
Duo.prototype.dependencies = function *(file, root, out) {
  ...
  var relativeFile = resolve(this.root, file);
  var mapping = this.mapping[relativeFile];
  var layer = out[relativeFile] = { deps: {} };
  var mtime = layer.mtime = (yield fs.stat(file)).mtime;
  ...
  if (mapping && mtime == mapping.mtime) {
    ...
    for (var dep in layer.deps) {
      file = layer.deps[dep];
      gens.push(this.dependencies(file, this.findroot(root), out));
    }
  ...
  for (var i = 0, dep; dep = deps[i]; i++) {
    resolved = this.resolve(dep, root);
    if (resolved) {
      layer.deps[dep] = resolved;
      gens.push(this.dependencies(resolved, root, out));
    } else {
      pkg = this.package(dep, json);
      if (!pkg) throw error('cannot resolve %s in %s', dep, file);
      pkgmap[dep] = pkg;
      pkgs.push(pkg);
    }
  }

  // TODO: 6) filter out packages that we already resolved
  ...
  for (var dep in pkgmap) {
    ...
    pkgroot = pkg.path();
    layer.deps[dep] = this.resolve(pkg.path(path), pkgroot);
    gens.push(this.dependencies(layer[dep], pkgroot, out));
  }
...
Duo.prototype.resolve = function(path, root) {
