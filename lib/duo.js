...
Duo.prototype.run = unyield(function *() {
  ...
  var mappingPath = this.installPath('duo.json');
  var mapping = Mapping(mappingPath);
...
Duo.prototype.dependencies = function *(file, out) {
  ...
  for (var i = 0, dep; dep = deps[i++];) {
    var val = includes[dep]
      ? dep
      : yield this.dependency(dep, file);
...
/**
 * Resolve a dependency by `dep`, from `parent` with an `entry`.
 *
 * @param {String} dep
 * @param {File} parent
 * @return {String}
...
Duo.prototype.dependency = function *(dep, parent) {
  var entry = this.entry();
  var isHTTP = http(dep);

  // ignore http dependencies
  if (isHTTP) {
    debug('%s: ignoring dependency "%s"', parent.id, dep);
    return false;
  }

  // local dependency
  var local = yield this.resolve(dep, parent, entry);
  if (local) return relative(entry.root, local);

  // remote dependency
  var manifest = this.manifest();
  var manifestPath = join(parent.root, manifest);
  var manifestJSON = readJson(manifestPath);
  var dependencyPackage = this.package(dep, manifestJSON);
  if (!dependencyPackage) {
    debug('%s: cannot resolve "%s"', parent.id, dep);
    return false;
  }
  ...
  // path
  var manifestPath = parse(dep).path || manifest;
  var dependencyManifestPath = dependencyPackage.path(manifestPath);
  var dependencyRoot = dependencyPackage.path();
  var dependencyManifestFile = this.file({
    root: dependencyRoot,
    path: dependencyManifestPath
  });
  var dependencyEntryPath = yield this.resolve(
    dependencyManifestPath,
    dependencyManifestFile
  );

  // logging
  if (!dependencyEntryPath) debug('%s: cannot resolve "%s"', parent.id, dep);
...
/**
 * Resolve local dependency into a path to a specific file.
 *
 * If it is a component.json, *.js, or *.css, then it's considered a dependency.
 * Otherwise it's considered an "asset".
 *
 * @param {String} dep
 * @param {String} root
 * @return {String} path
...
Duo.prototype.resolve = function *(dep, file) {
  var entry = this.entry();
